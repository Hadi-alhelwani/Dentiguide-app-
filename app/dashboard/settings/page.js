...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!data) {
        ...
        const { data: created } = await supabase.from("settings").insert({ user_id: user.id }).select().single();
        data = created;
      }
      setS(data);
    }
  ...
  const up = (k) => (e) => setS(prev => ({ ...prev, [k]: e.target.value }));

  if (!s) return <div className="p-8 text-gray-400">Loading...</div>;

  const Field = ({ label, field, span }) => (
    <div className={span === 2 ? "col-span-2" : ""}>
      <label className="block text-xs font-semibold text-gray-500 mb-1">{label}</label>
      <input value={s[field] || ""} onChange={up(field)}
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Company and signatory information — used on all documents.</p>
        </div>
      ...
      <div className="space-y-6">
        <section className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Postal Code" field="postal" />
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Full Name" field="prrc_name" />
            <Field label="Qualifications" field="prrc_qual" />
          </div>
        </section>
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Name" field="signer_name" />
            <Field label="Title / Function" field="signer_title" />
            <Field label="Credentials" field="signer_credentials" span={2} />
          </div>
          <p className="text-xs text-gray-400 mt-2">Credentials appear after your name on MDR Statement and Delivery Note signatures.</p>
        </section>

        <section className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="text-base font-semibold text-gray-800 mb-4">🏭 Additional Manufacturing Site (optional)</h2>
          <div className="grid grid-cols-2 gap-4">
            <Field label="Site Name" field="site2_name" />
            <Field label="Site Address" field="site2_address" />
          </div>
        </section>
      </div>
    </div>
