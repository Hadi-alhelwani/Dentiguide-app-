...
<div class="header"><div><h1>EU MDR 2017/745 — Annex XIII</h1><p>Custom-Made Dental Device Statement</p></div><div style="text-align:right"><div style="font-size:9px;font-weight:600;margin-bottom:2px">${esc(mfr.name)}</div><span class="tag">Custom-made device</span><span class="tag">EU MDR 2017/745</span><span class="tag">Annex XIII</span></div></div>
<div class="doc-ref-bar"><div>Ref: <strong>${docRef}</strong>${device.labRef?` &middot; Lab: <strong>${esc(device.labRef)}</strong>`:""}${prescriber.orderRef?` &middot; Rx: <strong>${esc(prescriber.orderRef)}</strong>`:""} &middot; Date: <strong>${fmtDate(sign.date)}</strong></div><div>Indicative: Class ${highestClass} &middot; Form v1.0</div></div>
...
<table class="mat-table"><thead><tr><th>Material</th><th>Manufacturer</th><th>Lot/Batch</th><th>CE</th><th>Expiry</th></tr></thead><tbody>${matRows.map(r=>`<tr><td>${esc(r.material)}</td><td>${esc(r.manufacturer)}</td><td>${esc(r.batch||"Per mfr records")}</td><td>${r.ceMarked?"✓":"✗"}</td><td>—</td></tr>`).join("")}</tbody></table>
...
<div style="border:1px solid #d0dbe8;border-radius:6px;padding:8px 12px;margin:5px 0"><div style="font-size:7px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;color:#4a6fa5;margin-bottom:4px">Biocompatibility Confirmation (Annex I GSPR)</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:2px;font-size:7.5px;line-height:1.5"><div>☑ CE-marked biocompatible materials used for intended purpose</div><div>☑ ISO 10993 biological safety covered by material manufacturer</div><div>☑ Manufacturer IFU followed</div><div>☑ No known allergens / hazards</div></div></div>
<div style="border:1px solid #d0dbe8;border-radius:6px;padding:8px 12px;margin:5px 0"><div style="font-size:7px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;color:#4a6fa5;margin-bottom:4px">Warnings &amp; Limitations</div><div style="font-size:7.5px;line-height:1.6"><div>● Single-use only. Reuse or reprocessing beyond one clinical use is not permitted.</div><div>● Intended for temporary intra-oral contact during the surgical procedure only.</div><div>● Intra-operative verification of correct seating, stability, and fit is the responsibility of the prescribing clinician.</div><div>● The manufacturer does not perform clinical assessment, surgical decision-making, or intra-operative adjustments.</div></div><div style="font-size:6.5px;color:#7a8fa5;text-align:right;margin-top:3px;font-style:italic">Case data consistency verified prior to manufacturing.</div></div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin:5px 0"><div style="border:1px solid #d0dbe8;border-radius:6px;padding:8px 12px"><div style="font-size:7px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;color:#4a6fa5;margin-bottom:4px">Substances / Tissues (Annex XIII §1(c))</div><div style="font-size:7.5px;line-height:1.6"><div>☐ Medicinal substance &nbsp;&nbsp; ☐ Human blood / plasma</div><div>☐ Human tissue / cells &nbsp;&nbsp; ☐ Animal tissue / cells</div><div>☑ None of the above</div></div></div><div style="border:1px solid #d0dbe8;border-radius:6px;padding:8px 12px"><div style="font-size:7px;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;color:#4a6fa5;margin-bottom:4px">Authorised Signatory</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:2px"><div><div style="font-size:6px;color:#7a8fa5">NAME (PRINT)</div><div style="font-size:8px;font-weight:600">${esc(sign.signerName)}${sign.credentials?`, ${esc(sign.credentials)}`:""}</div></div><div><div style="font-size:6px;color:#7a8fa5">TITLE / FUNCTION</div><div style="font-size:8px;font-weight:600">${esc(sign.signerTitle)}</div></div><div><div style="font-size:6px;color:#7a8fa5">DATE</div><div style="font-size:8px;font-weight:600">${fmtDate(sign.date)}</div></div><div><div style="font-size:6px;color:#7a8fa5">SIGNATURE</div><div style="border-bottom:1px dashed #999;min-height:18px"></div></div></div></div></div>
<div style="background:#f0f4f8;border-radius:4px;padding:4px 8px;margin-top:4px;font-size:6px;color:#5a6a7a;-webkit-print-color-adjust:exact;print-color-adjust:exact">📋 <strong>Retention:</strong> ${retention} after placing on market (Annex XIII §4). Report incidents to <strong>BfArM:</strong> medizinprodukte@bfarm.de &middot; ℹ Exempt from UDI (Art. 27(1)) &amp; CE marking (Art. 20(1)) as custom-made device.</div>
<div style="border-top:0.5px solid #e0e6ec;margin-top:3px;padding-top:2px;font-size:5.5px;color:#a0aab4;display:flex;justify-content:space-between"><span>${esc(mfr.name)} &middot; ${esc(mfr.city)}, ${esc(mfr.country)}</span><span>${docRef} &middot; EU MDR 2017/745 &middot; Form v1.0</span></div>
<div style="font-size:5px;color:#b0bac4;text-align:center;margin-top:1px">Controlled document — changes require version update &middot; Terminology: &ldquo;Custom-made device&rdquo; as defined in Article 2(3), Regulation (EU) 2017/745.</div></body></html>`;
  };
