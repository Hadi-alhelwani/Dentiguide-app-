    ...
    <aside className="fixed left-0 top-0 bottom-0 w-56 bg-slate-900 flex flex-col z-50">
      <div className="px-5 py-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-white font-bold text-sm">Dentiguide</div>
            <div className="text-slate-500 text-[10px] uppercase tracking-wider">MDR System</div>
          </div>
        </div>
      </div>
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg mb-0.5 text-sm transition-colors ${
                active ? "bg-slate-700/80 text-white font-semibold" : "text-slate-400 hover:text-white hover:bg-slate-800"
              }`}>
      ...
      <div className="px-3 py-4 border-t border-white/10">
        <button onClick={handleLogout}
          className="flex items-center gap-3 px-3 py-2 rounded-lg text-slate-500 hover:text-white hover:bg-slate-800 text-sm w-full transition-colors">
